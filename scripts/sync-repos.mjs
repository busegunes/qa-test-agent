...
/**
 * Commits that exist on `compareBranch` but not on the branch we track.
 *
 * Hotfixes land on main and are auto-PRed back to develop, so the two are
 * normally in sync. When an auto-PR hits a conflict it stalls, and a fix that
 * is live in production is missing from the branch under test. That gap is
 * worth knowing about before writing test cases.
 */
function readUnmerged(mirrorDir, trackedBranch, compareBranch) {
  if (!compareBranch || compareBranch === trackedBranch) return null;
  if (!tryGitIn(mirrorDir, ['rev-parse', '--verify', '--quiet', `${compareBranch}^{commit}`])) return null;

  const raw = tryGitIn(mirrorDir, [
    'log',
    '--no-merges',
    `--format=%h${US}%aI${US}%an${US}%s`,
    `${trackedBranch}..${compareBranch}`,
  ]);
  const commits = (raw || '')
    .split('\n')
    .filter(Boolean)
    .map((line) => {
      const [short, date, author, subject] = line.split(US);
      return { short, date, author, subject };
    });

  return { branch: compareBranch, count: commits.length, commits };
}
...
function syncRepo(project, repo, config) {
  ...
  const state = {
    ...
    unmerged: readUnmerged(mirrorDir, branch, repo.compareBranch),
    hotspots: readHotspots(mirrorDir, branch, config.hotspotWindowDays ?? 30),
    commits,
  };

  const drift = state.unmerged?.count
    ? ` — ${state.unmerged.count} commit(s) on ${state.unmerged.branch} not in ${branch}`
    : '';
  log.ok(`${label} — ${branch} @ ${state.head?.short ?? 'empty'} (${commits.length} commits recorded)${drift}`);
  return state;
}
...
function renderRepoSection(repo, windowDays) {
  ...
  if (repo.unmerged) {
    lines.push(`### Not Yet Merged from \`${repo.unmerged.branch}\``);
    lines.push('');
    if (repo.unmerged.count === 0) {
      lines.push(`\`${repo.branch}\` is up to date with \`${repo.unmerged.branch}\`.`);
      lines.push('');
    } else {
      lines.push(
        `**${repo.unmerged.count} commit(s) are on \`${repo.unmerged.branch}\` but not on \`${repo.branch}\`.** ` +
          `Hotfixes land on \`${repo.unmerged.branch}\` and are auto-PRed back; when that PR hits a conflict it stalls, ` +
          `so these fixes are live in production but missing from the branch under test.`
      );
      lines.push('');
      lines.push('Treat anything below as a known gap between production and what you are testing.');
      lines.push('');
      lines.push('| Commit | Date | Author | Subject |');
      lines.push('| --- | --- | --- | --- |');
      for (const commit of repo.unmerged.commits) {
        lines.push(`| \`${commit.short}\` | ${commit.date.slice(0, 10)} | ${commit.author} | ${commit.subject} |`);
      }
      lines.push('');
    }
  }
...
function renderSummary(states) {
  ...
  lines.push('');
  lines.push('| Project | Repository | Branch | HEAD | Last Commit | Unmerged | Synced |');
  lines.push('| --- | --- | --- | --- | --- | --- | --- |');
  const drifted = [];
  for (const { project, state } of states) {
    for (const repo of state.repos) {
      ...
      let unmerged = '—';
      if (repo.unmerged) {
        unmerged = repo.unmerged.count === 0 ? 'in sync' : `**${repo.unmerged.count} from \`${repo.unmerged.branch}\`**`;
        if (repo.unmerged.count > 0) drifted.push(`${project.key}/${repo.name}`);
      }
      lines.push(
        `| ${project.name} | \`${repo.name}\` | \`${repo.branch}\` | \`${repo.head.short}\` | ${repo.head.date.slice(0, 10)} — ${repo.head.subject} | ${unmerged} | ${state.syncedAt.slice(0, 10)} |`
      );
    }
  }
  lines.push('');
  if (drifted.length > 0) {
    lines.push(
      `**Branch drift:** ${drifted.join(', ')} ${drifted.length === 1 ? 'has' : 'have'} commits on the ` +
        'comparison branch that are missing from the branch under test. See the ' +
        '"Not Yet Merged" section in that project\'s `recent-changes.md` — those fixes are ' +
        'live in production but not in what you are testing.'
    );
    lines.push('');
  }
